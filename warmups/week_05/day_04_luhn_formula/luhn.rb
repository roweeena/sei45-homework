...
# declare our function accepts a number
def luhn num
  # .digits - ruby method: number => arrays of digits
  p array_digits = num.digits
  total = 0

  # loop through that array 
  array_digits.map.with_index do |digit, i|
    # double the number if its index is odd
    digit *= 2 if i.odd?
  
    # check if the number >= 10, subtract 9 from the number
    digit -= 9 if digit >= 10
